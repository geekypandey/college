        ...
        while((st = br.readLine()) != null){
			...
			for(i=1;i<arr.length;i++){
				...
				if(equal_index < 0) {
                    ...
                    //loop through the kptab to get the entry
                    //and get the value
                    int pp = mnt.get(macro_s).pp;
                    int kp_start = mnt.get(macro_s).kpdtp;
				    int kp_end = kp_start + mnt.get(macro_s).kp;
                    int j=0;
                    for(j=kp_end-(pp-bucket.size());j<kp_end;j++){
					    if((arr[i].substring(0,equal_index)).equals(kptab.get(j).key))
                        bucket.put((pp+j-kp_start+1),arr[i].substring(equal_index+1,arr[i].length()));
				    }
            	}
			}
			...
			if(bucket.size() != bucket_len){
				...
				for(i=kp_end-(bucket_len-bucket.size());i<kp_end;i++){
                    bucket.put(pp+i-kp_start+1,kptab.get(i).value);
				}				
			}
				...
				if(!mdt.get(i).equals("MEND")){
					...
					for(String x: final_split){
						...
						if(comma_index < 0){
							...
							int ind = Integer.parseInt(x.substring(comma_index+1,brac_index))  ;
